...
$(function () {
    ...
    var mainLoop = function () {
        ...
        if ( isClear ){
            ...
            context.strokeText("GAME CLEAR", WIDTH / 2, HEIGHT / 2);
            return;
        }
        ...
        if (gameover){
            context.textAlign = "center";
            context.strokeStyle = "red";
            context.strokeText("GAME OVER", WIDTH / 2, HEIGHT / 2);
            return;
        }
...
EnemyManager.prototype.draw = function (context) {
    ...
    var num = this.enemyList.some(function(enemy){
    ...
    this.enemyList.forEach(function (enemy) {
        if(num){
            enemy.enemyspeed *= -1;
            enemy.move(enemy.enemyspeed * 1, 45);
        }else {
            enemy.move(enemy.enemyspeed * 1, 0);
        }
        enemy.draw(context);

        if(enemy.pos.y + enemy.pos.y / 2 >= 450){
            gameover = true;
        }
